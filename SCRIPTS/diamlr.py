    ...
    for fld in flds:
        ...
        for reg in regs:
            ...
            for nc_att in nc_gridvar.ncattrs():
                if nc_att in ['_FillValue']:
                    nc_var_fill = nc_gridvar.getncattr(nc_att)
                elif nc_att in ['missing_value']:
                    nc_var_fill = nc_gridvar.getncattr(nc_att)
            print(nc_var_fill)            
            nc_var = nc_out.createVariable(name[0]+'-'+name[1],
                                           nc_gridvar.datatype,
                                           nc_gridvar.dimensions, fill_value=nc_var_fill)
            print(nc_var)
